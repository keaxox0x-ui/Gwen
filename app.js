...
function normalizeRewardData(){
  ...
  // First-run test setup: one real album + enough coins to test the draw button.
  // It runs only once and never overwrites existing album progress.
  if(!data.sampleRewardSeeded){
    if(Object.keys(data.albums).length === 0){
      data.albums[1] = 1;
      if(data.coins < 100) data.coins = 100;
    }
    data.sampleRewardSeeded = true;
    saveData();
  }
}
...
function renderReward(){
  ...
  const modalBalance=document.querySelector("#modalCoinBalance");
  if(modalBalance) modalBalance.textContent=String(data.coins);
...
function lockedCoverHTML(className="album-cover"){
  return `<div class="${className} placeholder locked-cover">LOCKED</div>`;
}
...
function renderAlbumCollection(){
  ...
  el.innerHTML=ALBUMS.map(album=>{
    ...
    return `<div class="album-card ${level?"":"locked"}">
      ${level ? coverHTML(album) : lockedCoverHTML()}
...
const closeAlbumButton=document.querySelector("#closeAlbumButton");
function openAlbumModal(){albumModal.classList.add("open");albumModal.setAttribute("aria-hidden","false");document.body.classList.add("modal-open");}
function closeAlbumModal(){albumModal.classList.remove("open");albumModal.setAttribute("aria-hidden","true");document.body.classList.remove("modal-open");}
openAlbumButton.addEventListener("click",openAlbumModal);
closeAlbumButton.addEventListener("click",closeAlbumModal);
albumModal.addEventListener("click",e=>{if(e.target.hasAttribute("data-close-album"))closeAlbumModal()});
document.addEventListener("keydown",e=>{if(e.key==="Escape" && albumModal.classList.contains("open"))closeAlbumModal()});
